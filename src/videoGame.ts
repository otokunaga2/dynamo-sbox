...
export async function searchWithHashKey(hashkey) {
	const params: AWS.DynamoDB.DocumentClient.QueryInput = {
		TableName: 'Games',
		Limit: 1,
...
export async function documentClient_batchWrite() {
	const params: AWS.DynamoDB.DocumentClient.BatchWriteItemInput = {
		RequestItems: {
			'Games': [
				{
					PutRequest: { Item: { Hardware: 'FC', GameId: '1984-lode-runner', Title: 'Lode Runner', Players: 1, Genre: 'ACT', SavedTimeStamp: new Date().getTime() } }
				},
				{
					PutRequest: { Item: { Hardware: 'FC', GameId: '1985-exed-exes', Title: 'Exed Exes', Players: 1, Genre: 'STG', SavedTimeStamp: new Date().getTime() } }
				},
				{
...
export async function updateBatch(){
  ...
  TableName: 'Games',
  IndexName: 'statusIndex',
  KeyConditionExpression: '#approvalStatus = :approvalStatus AND #companyName = :indexValue',
   ExpressionAttributeNames: {
